...
if ($RGStatus1 -eq "")
{
    ...
    az network bastion create --location $RGLocation --name $RBastionName --public-ip-address $MyIp --resource-group $RGName --vnet-name $RVnetName  --sku
}
else {
    write-Host Network $RGName, $RBastionName already exists   
